...
function AuthorizeHandler(options) {
  ...
  if (!options.model.saveAuthCode) {
    throw new InvalidArgumentError('Invalid argument: model does not implement `saveAuthCode()`');
  }
...
AuthorizeHandler.prototype.handle = function(request, response) {
  ...
  return Promise.all(fns)
    .bind(this)
    .spread(function(authCode, expiresAt, client, scope, state, user) {
      return this.saveAuthCode(authCode, expiresAt, scope, client, user)
        .bind(this)
        .then(function(code) {
...
AuthorizeHandler.prototype.saveAuthCode = Promise.method(function(authCode, expiresAt, scope, client, user) {
  var code = {
    authCode: authCode,
    expiresAt: expiresAt,
    scope: scope
